...
class YjsBridge:
    ...
    async def get_active_notebook(
        ...
        """
        result = await self.list_notebooks(params, notebook_id)
    ...
    async def get_notebook_content(
        ...
        if self._contents_manager:
            try:
                model = await self._contents_manager.get(path, content=True)
    ...
    async def get_cell(
        ...
        content = await self.get_notebook_content(params, notebook_id)
        cells = content.get("cells", [])
    ...
    async def insert_cell(
        ...
        return await self._modify_notebook(
            path,
            "insert",
    ...
    async def update_cell(
        ...
        return await self._modify_notebook(path, "update", cell_index=cell_index, source=source)
    ...
    async def delete_cell(
        ...
        return await self._modify_notebook(path, "delete", cell_index=cell_index)
    ...
    async def move_cell(
        ...
        return await self._modify_notebook(path, "move", from_index=from_index, to_index=to_index)

    async def _modify_notebook(self, path: str, operation: str, **kwargs: Any) -> dict[str, Any]:
        ...
        content["cells"] = cells
        await self._contents_manager.save(
    ...
    async def execute_cell(
        ...
        # Get cell content
        cell_data = await self.get_cell(params, path)
        source = cell_data["cell"]["source"]

        # Execute the code
        return await self.execute_code({"code": source, "path": path}, path)
    ...
    async def execute_code(
        ...
        # client.execute may be sync or async depending on client type
        msg_id = await maybe_await(client.execute(code))
        outputs: list[dict[str, Any]] = []

        while True:
            try:
                # get_iopub_msg may be sync or async depending on client type
                msg = await maybe_await(client.get_iopub_msg(timeout=30))
    ...
    async def interrupt_kernel(
        ...
        for session in sessions:
            if session.get("path") == path:
                kernel_id = session.get("kernel", {}).get("id")
                if kernel_id:
                    # interrupt_kernel may be sync or async
                    await maybe_await(self._kernel_manager.interrupt_kernel(kernel_id))
    ...
    async def get_kernel_status(
        ...
        for session in sessions:
            if session.get("path") == path:
                ...
                if kernel_id:
                    kernel = self._kernel_manager.get_kernel(kernel_id)
                    # is_alive() may be sync or async depending on kernel manager
                    is_alive = await maybe_await(kernel.is_alive())
                    return {
                        "kernel_id": kernel_id,
...
"""
    ...
    async def get_cell_outputs(
        ...
        Args:
            ...
            notebook_id: Notebook path.
        """
        cell_data = await self.get_cell(params, notebook_id)
